...
export default function EmployeePage() {
  const { isLoading, user, signOut } = useAuthPage("employee");

  if (isLoading) {
    return (
      <main className="flex min-h-screen items-center justify-center text-center">
        <p style={{ color: "var(--text-muted)" }}>Loading your employee portal...</p>
      </main>
    );
  }
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8 text-center">
      <div
        className="w-full max-w-2xl space-y-4 rounded-[var(--radius-lg)] border p-8"
        style={{
